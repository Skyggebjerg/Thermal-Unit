...
const byte MLX90640_address =
    0x33;  //Default 7-bit unshifted address of the MLX90640.  MLX90640的默认7位未移位地址
#define TA_SHIFT \
...
void loop() {
  ...
  for (byte x = 0; x < speed_setting; x++)  // x < 2 Read both subpages
  {
    ...
    float tr =
        Ta -
        TA_SHIFT;  //Reflected temperature based on the sensor ambient temperature.  根据传感器环境温度反射温度
    float emissivity = 0.95;
    MLX90640_CalculateTo(
        mlx90640Frame, &mlx90640, emissivity, tr,
        pixels);  //save pixels temp to array (pixels).  保存像素temp到数组(像素)
  ...
  if (max_v > max_cam_v | max_v < min_cam_v) {
    ...
    M5.Lcd.printf("C");
    M5.Lcd.drawCircle(160, 120, 6,
                      TFT_WHITE);  // update center spot icon.  更新中心点图标
